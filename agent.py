...
class DQNModel(nn.Module):
    ...
    def __init__(self, board_size, n_frames, n_actions, model_config):
        ...
        for layer_name, layer_config in model_config.items():
            if 'Conv2D' in layer_name:
                ...
                # We'll handle dense layers after knowing the flattened size
                pass
        
        self.conv_layers = nn.Sequential(*layers[:-1]) if layers else nn.Identity()
        ...
        # Final action values layer
        dense_layers.append(nn.Linear(in_features, n_actions))
        self.dense_layers = nn.Sequential(*dense_layers)
        
    def forward(self, x):
        # Input: (batch, board_size, board_size, n_frames)
...
class DeepQLearningAgent(Agent):
    ...
    def _get_model_outputs(self, board, model=None):
        ...
        board_tensor = torch.FloatTensor(board).to(self._device)
    ...
    def _agent_model(self):
        ...
        model : PyTorch Model
            DQN model
        """
        # define the model architecture from config file
        with open('model_config/{:s}.json'.format(self._version), 'r') as f:
            m = json.loads(f.read())
        
        model = DQNModel(self._board_size, self._n_frames, self._n_actions, m['model'])
        
        return model

    def set_weights_trainable(self):
        """Set selected layers to non trainable
        In PyTorch, we can freeze specific layers by setting requires_grad
        """
        # Freeze all layers first
        for param in self._model.parameters():
            param.requires_grad = False
        
        # Make final dense layers trainable
        # This assumes the last few layers are the action value layers
        for param in list(self._model.dense_layers.parameters())[-4:]:
    ...
    def train_agent(self, batch_size=32, num_games=1, reward_clip=False):
        ...
        -------
            loss : float
            The current error (error metric is huber loss)
        """
        ...
        # Convert to tensors
        s_tensor = torch.FloatTensor(self._normalize_board(s)).to(self._device)
        target_tensor = torch.FloatTensor(target).to(self._device)
        
        # Train
        self._model.train()
        self._optimizer.zero_grad()
        predictions = self._model(s_tensor)
        loss = mean_huber_loss(target_tensor, predictions)
        loss.backward()
    ...
    def compare_weights(self):
        ...
        for i, (model_param, target_param) in enumerate(zip(model_params, target_params)):
            match = torch.equal(model_param, target_param)
            print('Parameter {:d} Match : {:d}'.format(i, int(match)))
...
class PolicyGradientAgent(DeepQLearningAgent):
    ...
    def _agent_model(self):
        ...
        """
        model = PolicyGradientModel(self._board_size, self._n_frames, self._n_actions)
        return model
    ...
    def train_agent(self, batch_size=32, beta=0.1, normalize_rewards=False,
        ...
        # normalize the rewards for training stability
        if(normalize_rewards):
            r = (r - np.mean(r))/(np.std(r) + 1e-8)
        
        # Convert to tensors
        s_tensor = torch.FloatTensor(self._prepare_input(s)).to(self._device)
        a_tensor = torch.FloatTensor(a).to(self._device)
        r_tensor = torch.FloatTensor(r).to(self._device)
        
        # Train
        self._model.train()
        self._actor_optimizer.zero_grad()
        
        logits = self._model(s_tensor)
        policy = F.softmax(logits, dim=1)
        log_policy = F.log_softmax(logits, dim=1)
        
        # Calculate loss
        advantage = a_tensor * r_tensor
        ...
        return [actor_loss.item(), -J.item(), entropy.item()]

class A2CModel(nn.Module):
    """PyTorch model for Advantage Actor Critic"""
    def __init__(self, board_size, n_frames, n_actions):
        super(A2CModel, self).__init__()
        self.conv1 = nn.Conv2d(n_frames, 16, kernel_size=3)
...
class AdvantageActorCriticAgent(PolicyGradientAgent):
    ...
    Attributes
    ----------
    _full_model : PyTorch Model
        Contains the complete A2C model with both actor and critic heads
    ...
    def _agent_model(self):
        """Returns the model which evaluates prob logits and action values 
        for a given state input
        Overrides parent
        ...
        model : PyTorch Model
            A2C model
        """
        model = A2CModel(self._board_size, self._n_frames, self._n_actions)
        return model
    ...
    def train_agent(self, batch_size=32, beta=0.001, normalize_rewards=False,
        ...
        # Convert to tensors
        s_tensor = torch.FloatTensor(s_prepared).to(self._device)
        next_s_tensor = torch.FloatTensor(next_s_prepared).to(self._device)
        a_tensor = torch.FloatTensor(a).to(self._device)
        r_tensor = torch.FloatTensor(r).to(self._device)
        done_tensor = torch.FloatTensor(done).to(self._device)
        
        # calculate V values
        self._model.eval()
        with torch.no_grad():
            if(self._use_target_net):
                _, next_s_pred = self._target_net(next_s_tensor, return_values=True)
            else:
                _, next_s_pred = self._model(next_s_tensor, return_values=True)
        
        # prepare target
        future_reward = self._gamma * next_s_pred * (1 - done_tensor)
        
        # Train
        self._model.train()
        self._optimizer.zero_grad()
        
        logits, s_pred = self._model(s_tensor, return_values=True)
        policy = F.softmax(logits, dim=1)
        log_policy = F.log_softmax(logits, dim=1)
        
        # calculate target for actor (uses advantage), similar to Policy Gradient
        advantage = a_tensor * (r_tensor + future_reward - s_pred)
        
        # calculate target for critic, simply current reward + future expected reward
        critic_target = r_tensor + future_reward
        
        # calculate loss
        J = torch.sum(advantage * log_policy) / num_games
        entropy = -torch.sum(policy * log_policy) / num_games
        actor_loss = -J - beta * entropy
        critic_loss = mean_huber_loss(critic_target, s_pred)
...
class SupervisedLearningAgent(DeepQLearningAgent):
    ...
    Attributes
    ----------
    _model_action_optimizer : torch.optim.Optimizer
        Optimizer for supervised learning training
    """
    def __init__(self, board_size=10, frames=2, buffer_size=10000,
                 gamma=0.99, n_actions=3, use_target_net=True,
                 version=''):
        """Initializer for SupervisedLearningAgent, similar to DeepQLearningAgent
        but creates extra optimizer for classification training
        """        
        ...
        self._model_action_optimizer = optim.Adam(self._model.parameters(), lr=0.0005)
        
    def train_agent(self, batch_size=32, num_games=1, epochs=5, 
                    reward_clip=False):
        """Train the model by sampling from buffer and return the error.
        Model is trained as a classification problem to learn weights
        ...
        for epoch in range(epochs):
            self._model_action_optimizer.zero_grad()
            logits = self._model(s_tensor)
            # Use cross entropy loss (expects class probabilities vs one-hot targets)
            loss = F.cross_entropy(logits, a_tensor.argmax(dim=1))
            loss.backward()
            self._model_action_optimizer.step()
            losses.append(loss.item())
        
        return round(losses[-1], 5)
    ...
    def normalize_layers(self, max_value=None):
        ...
        # Get the final action values layer (last layer in dense_layers)
        final_layer = list(self._model.dense_layers.children())[-1]
        if isinstance(final_layer, nn.Linear):
            with torch.no_grad():
                final_layer.weight.data /= max_value
                if final_layer.bias is not None:
                    final_layer.bias.data /= max_value
